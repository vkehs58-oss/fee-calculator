  ...
  return (
    <div className="px-4 mt-8">
      ...
      <div className="bg-white rounded-[16px] shadow-sm overflow-hidden">
        ...
        {/* 범례 */}
        <div className="border-t border-toss-gray-100 px-4 py-2.5">
          <div className="flex flex-wrap gap-x-4 gap-y-1">
            {platformSettlements.map((p) => (
              <div key={p.short} className="flex items-center gap-1">
                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: p.color }} />
                <span className="text-[10px] text-toss-gray-400">{p.name} {p.rule}</span>
              </div>
            ))}
            <div className="flex items-center gap-1">
              <div className="w-2 h-2 rounded-full bg-toss-red" />
              <span className="text-[10px] text-toss-gray-400">공휴일</span>
            </div>
          </div>
        </div>
      </div>
